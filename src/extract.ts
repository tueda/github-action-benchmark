...
async function getCommitFromRefRepository(ref: string, refRepository: string): Promise<Commit> {
    ...
    const repo_dir = path.join(temp_dir, 'github-action-benchmark');
    try {
        await git.cmd(['clone', `${refRepository}.git`, temp_dir]);
        await git.cmd(['-C', repo_dir, 'checkout', ref]);

        const [commitHash, authorName, authorEmail, committerName, committerEmail, commitMessage] = await Promise.all([
            git.cmd(['-C', repo_dir, 'log', '-1', '--pretty=format:%H']),
            git.cmd(['-C', repo_dir, 'log', '-1', '--pretty=format:%an']),
            git.cmd(['-C', repo_dir, 'log', '-1', '--pretty=format:%ae']),
            git.cmd(['-C', repo_dir, 'log', '-1', '--pretty=format:%cn']),
            git.cmd(['-C', repo_dir, 'log', '-1', '--pretty=format:%ce']),
            git.cmd(['-C', repo_dir, 'log', '-1', '--pretty=format:%B']),
        ]);
